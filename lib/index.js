...
const _highlightText = (content, regexMatch, colorFunction) => {
    ...
    // let regii = new RegExp(regexMatch);

    var elements = content.match(regexMatch);
    ...
    if (elements !== null) {
        ...
        elements.forEach(element => {
            ...
            let find = new RegExp(element, 'igs');
            console.log(find);

            // replace it in content
            content = content.replace(find, colorFunction(element));
...
const _addHyperlinks = (content) => {
    ...
    elements.forEach(element => {
        ...
        let newHyperlink = element.substr(1, element.length - 1).replace(']', ' ');
        let internalLink = newHyperlink.match(/\(\.(.*?)\)/ig);

        if (internalLink !== null) {

            internalLink.forEach(element => {
                newHyperlink = newHyperlink.replace(element, '');
            })
